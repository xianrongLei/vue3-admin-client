...
async function login() {
  ...
  const [signInErr, data] = await awaitTo(signInApi.mutate());
  if (signInErr) {
  ...
  if (!data?.data?.signIn) throw new Error("登录失败");

  const { access_token, refresh_token, user } = data?.data?.signIn || null;
  // 缓存token 获取数据
